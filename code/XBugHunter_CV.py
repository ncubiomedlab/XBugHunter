...
for BAC in range(0, len(bac_abx['bac'])):
    ...
    for train_index, test_index in kf.split(X, y):
        ...
        # 根據k-fold的測試的資料找到Optimal Cuttoff
        ...
        if fold == 1:
            ...
            pred_list_DT = y_pred_prob_DT.tolist()
            pred_list_RF = y_pred_prob_RF.tolist()
            pred_list_LR = y_pred_prob_LR.tolist()
            pred_list_SVM = y_pred_prob_SVM.tolist()
            pred_list_NN = y_pred_prob_NN.tolist()
        else:
            true_label.extend(y_test.tolist())
            pred_list_DT.extend(y_pred_prob_DT.tolist())
            pred_list_RF.extend(y_pred_prob_RF.tolist())
            pred_list_LR.extend(y_pred_prob_LR.tolist())
            pred_list_SVM.extend(y_pred_prob_SVM.tolist())
            pred_list_NN.extend(y_pred_prob_NN.tolist())
        ...
        print("========================================")
        Folds = Folds + [fold]*len(y_pred_prob_NN)
        fold = fold + 1
    ...
    final.to_csv(output_pth + FileName + '_CV.csv', index=False)

    final_prob = pd.DataFrame()
    final_prob['Fold'] = Folds
    final_prob['Pred_prob_DT'] = pred_list_DT
    final_prob['Pred_prob_RF'] = pred_list_RF
    final_prob['Pred_prob_LR'] = pred_list_LR
    final_prob['Pred_prob_SVM'] = pred_list_SVM
    final_prob['Pred_prob_NN'] = pred_list_NN
    final_prob['Label'] = true_label

    final_prob.to_csv(output_pth + FileName + '_Pred_probs.csv', index=False)

    print("*****************************************")
    print(FileName)
    print("%.4f" % np.array(auc_DT).mean())
    print("%.4f" % np.array(auc_RF).mean())
    print("%.4f" % np.array(auc_LR).mean())
    print("%.4f" % np.array(auc_SVM).mean())
    print("%.4f" % np.array(auc_NN).mean())
    print("*****************************************")
